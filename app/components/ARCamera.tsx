...
export default function ARCamera({ className = '' }: ARCameraProps) {
    ...
    const initFaceMesh = async () => {
      try {
        ...
        faceMesh.onResults((results) => {
          ...
          // 비디오 프레임 그리기 (Webcam이 이미 그려주므로 생략 가능)
  ...
  // 자동 분석 흐름 타이머
  useEffect(() => {
    if ((isCameraReady || isMockMode) && !isCameraLoading && !cameraError) {
      // 3초 후 스캔 완료 → 분석 시작
  ...
  }, [isCameraReady, isMockMode, isCameraLoading, cameraError, router])
